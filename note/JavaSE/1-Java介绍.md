...
>在IDEA中，可以选中变量之后，使用 `shift + f6` 批量修改变量名

## 三、基本数据类型

Java语言是**强类型语言**，对于每一种数据都定义了明确的具体数据类型，在内存总分配了不同大小的**内存空间**。
...
被使用的时候，++和--放在前面，则`先自增（或自减）`再`使用`，++和--放在后面，则`先使用`，再`自增（或自减）`。
...
比较之后的结果只有两种情况：`true、false`，比较运算的结果是`boolean`类型。
...
与 & 和 | 不同, & 和 | `不会`出现短路，即使`前半段表达式`就能确定整个式子最终结果，后半段代码`仍然执行`。单个的 `&` 和 `|` 还可以进行位运算，这是后面的内容。
